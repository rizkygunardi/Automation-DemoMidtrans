...
describe("Testing e2e Finaltest Eduwork", () => {
  ...
  it("Testing TC001 BRI PASS", () => {
    ...
    cy.get("#vaField").then(($temp) => {
      const txt = $temp.text();
      cy.wait(500);
      cy.visit("https://simulator.sandbox.midtrans.com/bri/va/index");
      cy.get("h1").should("contain.text", "BRI Virtual Account");
    ...
    cy.get("body > :nth-child(2)").should("include.text", "Success");
  });
  ...
  it("Testing TC002 BRI Fail", () => {
    ...
    cy.get("#vaField").then(($temp) => {
      ...
      cy.get(".alert").should(
        "contain.text",
        "Inquiry fails with bill status code = 2 / Inquiry gagal dengan bill status code = 2"
      );
    });
  });
  ...
  it("Testing TC003 BNI PASS", () => {
    ...
    cy.get('[href="#/bank-transfer/bni-va"] > .payment-page-text').click();
    cy.wait(5000);
    cy.get("#vaField").then(($temp) => {
      const txt = $temp.text();
      cy.wait(500);
  ...
  it("Testing TC005 Gopay PASS", () => {
    cy.Bio3();
    cy.BtnPay();
    cy.get(':nth-child(10) > [data-testid="list-item"]').click();
    cy.wait(5000);
    cy.get(".title-text").should("contain.text", "GoPay");
  ...
  it("Testing TC007 Akulaku PASS", () => {
    cy.Bio4();
    cy.BtnPay();
    cy.get(':nth-child(11) > [data-testid="list-item"]').click();
    cy.get(".page-title").should("contain.text", "Akulaku PayLater");
    cy.visit(
  ...
  it("Testing TC009 Indomart PASS", () => {
    cy.Bio5();
    cy.BtnPay();
    cy.get(':nth-child(9) > [data-testid="list-item"]').click();
    cy.get("#payment-code").then(($temp) => {
      const txt = $temp.text();
      cy.wait(500);
